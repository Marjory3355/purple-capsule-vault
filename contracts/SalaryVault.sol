...
contract SalaryVault is SepoliaConfig {
    ...
    function submitSalary(externalEuint32 encryptedSalary, bytes calldata inputProof, string memory position) external {
    ...
    function updateSalary(
        ...
        _encryptedPositionTotal[oldPositionHash] = FHE.sub(
            _encryptedPositionTotal[oldPositionHash],
            oldEncryptedSalary
        );
        _positionCount[oldPositionHash]--;

        // Clean up if position count reaches zero
        // Note: Cannot delete euint32, just leave it as zero-equivalent
    ...
    function getEntry(
        uint256 entryId
    ) external view returns (string memory position, uint256 timestamp, address submitter, bool isActive) {
        SalaryEntry storage entry = salaryEntries[entryId];
    ...
    function getEncryptedPositionStats(
        string memory position
    ) external view returns (euint32 encryptedTotal, uint32 count) {
        require(bytes(position).length > 0, "Position cannot be empty");
        bytes32 positionHash = keccak256(bytes(position));
    ...
    /// @dev Prevents reentrancy by marking state as completed before external interactions
    function positionStatsCallback(
        uint256 requestId,
        bytes memory cleartexts,
        bytes[] memory /*signatures*/
    ) public returns (bool) {
    ...
    /// @notice Mock decryption for localhost testing only
    /// @dev This function bypasses the decryption oracle for local development
    /// @param mockTotalSalary The mock total salary value to use
    function mockGlobalStatsCallback(uint32 mockTotalSalary) external {
        require(_activeEntryCount > 0, "No active entries");
        require(!_statsFinalized, "Stats already finalized");

        // Calculate average
        _decryptedAverageSalary = mockTotalSalary / _activeEntryCount;
        _statsFinalized = true;

        emit StatsPublished(_decryptedAverageSalary, _activeEntryCount);
    }

    /// @notice Mock position stats decryption for localhost testing only
    /// @dev This function bypasses the decryption oracle for local development
    /// @param position Position name
    /// @param mockTotalSalary The mock total salary value for this position
    function mockPositionStatsCallback(string memory position, uint32 mockTotalSalary) external {
        bytes32 positionHash = keccak256(bytes(position));
        require(_positionCount[positionHash] > 0, "No data for this position");
        require(!_positionFinalized[positionHash], "Position stats already finalized");

        uint32 count = _positionCount[positionHash];
        _decryptedPositionAverage[positionHash] = mockTotalSalary / count;
        _positionFinalized[positionHash] = true;

        emit PositionStatsPublished(positionHash, _decryptedPositionAverage[positionHash], count);
    }

    /// @notice Reset global stats (for testing purposes)
    /// @dev Allows re-testing decryption flow
    function resetGlobalStats() external {
        _statsFinalized = false;
        _decryptedAverageSalary = 0;
    }

    /// @notice Reset position stats (for testing purposes)
    /// @param position Position name
    function resetPositionStats(string memory position) external {
        bytes32 positionHash = keccak256(bytes(position));
        _positionFinalized[positionHash] = false;
        _decryptedPositionAverage[positionHash] = 0;
    }
}
